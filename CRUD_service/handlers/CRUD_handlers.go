...
func GetCategorieByDocenteAndNome(c *gin.Context) {
	...
	if idDocente == "" || categoria == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "ID del docente mancante o nome della categoria mancante"})
		return
	}
	var categorie []models.Categoria
	if err := database.DB.Where("id_docente = ? & nome = ?", idDocente, categoria).Preload("Docente").Find(&categorie).Error; err != nil {
		fmt.Println("Errore nel recupero delle categorie per il docente e il nome specificato:", err.Error())
